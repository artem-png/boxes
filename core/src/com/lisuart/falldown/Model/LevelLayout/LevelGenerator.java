...
public class LevelGenerator {
    ...
    public LevelGenerator(int beginLevel) {
        ...
        levelCount = com.lisuart.falldown.Config.Progress.levels.length;
    ...
    public static void init() {
        ...
        xd = (float) Gdx.graphics.getWidth() / (float) com.lisuart.falldown.MyGdxGame.width;
        yd = (float) Gdx.graphics.getHeight() / (float) com.lisuart.falldown.MyGdxGame.width;
    }
        ...
        for (int i = 4; i > 0; i--) {
            for (int j = 0; j < 3; j++) {
                ...
                if (a <= levelCount) {
                    ...
                    batch.setColor(c);
                    if (com.lisuart.falldown.Config.Progress.levels[a - 1] == 1) {
                        batch.draw(
                                crown,
                                j * 13 + 9.5f,
                                i * 11 + 17.5f,
    ...
    public void act() {
        if (isPressed) {
            timePressed--;
            if (timePressed <= 0) {
                com.lisuart.falldown.MyGdxGame.layoutManager.set(new GameLayout(com.lisuart.falldown.Config.Progress.getLevelByInt(chosenlevel)));
            }
        }
        int a = firstLevel;
        for (int i = 4; i > 0; i--) {
                    ...
                    if (Gdx.input.justTouched()) {
                        ...
                        if (vector3.x > j * 13 + 5.5f && vector3.x < j * 13 + 5.5f + 8) {
                            if (vector3.y > i * 11 + 13 && vector3.y < i * 11 + 13 + 8) {
                                chosenlevel = a;
                                isPressed = true;
                                com.lisuart.falldown.MyGdxGame.setUp(15, true);
                            }
                        }
                    }
                    a++;
