...
public class PowerShield extends APower {
    Vector2 size = new Vector2(com.lisuart.falldown.MyGdxGame.width * 0.75f, 1.4f);
    Vector2 position = new Vector2(com.lisuart.falldown.MyGdxGame.width + size.x, 18);
    ...
    public void input() {
        if (delay == 0 && !shieldUp && !shieldDown) {
            for (int i = 0; i <= 1; i++) {
                if (Gdx.input.justTouched() && Gdx.input.isTouched(i)) {
                    ...
                    if (vector3.x > com.lisuart.falldown.MyGdxGame.width - 7 && vector3.y > 27 && vector3.y < 39) {
                        isActive = true;
                        body.setActive(true);
                        shieldDown = true;
                    }
        ...
        if (isActive) {
            com.lisuart.falldown.Config.Tex.createParticles(1, 0.4f, new Vector2(random.nextInt(com.lisuart.falldown.MyGdxGame.width), body.getPosition().y - size.y/2));

            com.lisuart.falldown.Config.Tex.createParticles(6, 0.5f, new Vector2(body.getPosition().x + size.x, body.getPosition().y - size.y/3));
    ...
    public void render(SpriteBatch batch) {
        ...
        batch.draw(
                ...
                size.x * 2,
                size.y * 2,
                com.lisuart.falldown.Config.Tex.shield.getScaleX(),
                com.lisuart.falldown.Config.Tex.shield.getScaleY(),
                0
        );
    }
}
