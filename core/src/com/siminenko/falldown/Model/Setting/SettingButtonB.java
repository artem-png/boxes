...
public class SettingButtonB {
    ...
    public void act() {
        if (isPressed) {
            timePressed--;
            if (timePressed <= 1) {
                com.siminenko.falldown.Config.Progress.setMusic(!com.siminenko.falldown.Config.Progress.music);
                if (com.siminenko.falldown.Config.Progress.music) {
                    Music.music();
                } else {
                    Music.stopMusic();
                }
    ...
    public void render(SpriteBatch batch) {
        batch.draw(
                com.siminenko.falldown.Config.Progress.music ? button : buttonPressed,
                position.x - size.x/2,
                position.y - size.y/2,
                size.x,
                size.y
